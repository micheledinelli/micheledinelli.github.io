---
title: "Esercizi sui grafi"
description: "Esercizi sui grafi"
summary: "Estratto di una lezione contenente esercizi sulla teoria dei grafi."
...
La risposta è affermativa in entrambi i casi. Per la visita in profondità si può ad esempio partire da A oppure B. Per la visita in ampiezza si può partire ad esempio da F. In entrambi i casi (al di là del nodo di partenza, che è differente per la visita DFS e BFS) si può ottenere l'albero di visita mostrato nel testo utilizzando, ad esempio, la seguente rappresentazione con liste di adiacenza:

![image](sol-es-1.png)

## Esercizio 2

Scrivere un algoritmo efficiente per calcolare il numero di cammini minimi distinti che vanno da un nodo sorgente \\( s \\) a ciascun nodo \\( u \\) in un grafo non orientato, non pesato e connesso \\( G = (V, E) \\). Due cammini si considerano diversi se differiscono per almeno un arco; si noti che esiste un cammino minimo (il cammino vuoto) dal nodo \\( s \\) a se stesso. Applicare l'algoritmo al seguente grafo:
...
Ad esempio, consideriamo la situazione seguente in cui i nodi \\(x, y, z\\) si trovano a distanza \\(k \ \text{da} \ s\\), e sono collegati con un arco al nodo \\(v\\) che si trova a distanza \\(k + 1\\). Supponendo di aver già calcolato i numeri \\(c[x], c[y], c[z] \\) di cammini minimi distinti da \\(s\\) a (rispettivamente) \\(x, y, z\\), allora possiamo raggiungere \\(v\\) mediante \\(c[x]\\) cammini distinti che passano per \\(x\\), \\(c[y]\\) cammini distinti che passano per \\(y\\) e \\(c[z]\\) cammini minimi distinti che passano per \\(z\\).

![image](sol-es-2.0.png)

È possibile calcolare il valore \\(c[v]\\) per ogni nodo \\(v\\) mano a mano che il grafo viene visitato. Si pone inizialmente \\(c[v] \leftarrow 0 \ \forall v\\), ad eccezione della sorgente \\(s\\) per cui si pone \\(c[s] \leftarrow 1\\) in quanto esiste un solo cammino minimo (il cammino vuoto) dalla sorgente a se stessa. Ogni volta che l'algoritmo BFS attraversa l'arco non orientato \\(\{u, v\}\\) che conduce dal nodo \\(u\\) ad un nodo \\(v\\) a distanza \\(d[v] = d[u] + 1\\), si imposta \\(c[v] \leftarrow c[v] + c[u]\\).
